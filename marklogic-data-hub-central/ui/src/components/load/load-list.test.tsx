...
describe('Load data component', () => {
  ...
  test('Verify Load settings from list view renders correctly', async () => {
    ...
    fireEvent.change(targetPermissions, { target: { value: 'role1' }}); // BAD permission
    await wait(() => {
    ...
    fireEvent.change(targetPermissions, { target: { value: 'role1,reader' }}); // BAD permission
    await wait(() => {
        fireEvent.click(saveButton[1]);
    });
    expect(getByText(AdvancedSettingsMessages.targetPermissions.invalidCapabilities)).toBeInTheDocument();
    fireEvent.change(targetPermissions, { target: { value: ' ' }}); // BAD permission
    await wait(() => {
        fireEvent.click(saveButton[1]);
    });
    expect(getByText(AdvancedSettingsMessages.targetPermissions.incorrectFormat)).toBeInTheDocument();
    fireEvent.change(targetPermissions, { target: { value: 'role1,read' }}); // GOOD permission
    await wait(() => {
        fireEvent.click(saveButton[1]);
    });
    expect(queryByText(AdvancedSettingsMessages.targetPermissions.incorrectFormat)).not.toBeInTheDocument();
